...
class RemoveMemberCommand implements Command {
    public String execute(String[] parts, Database database) throws Exception {
        ...
        Person memberToRemove = group.getMembers().stream()
                ...
                .findFirst()
                .orElseThrow(() -> new PersonNotExistsException(String.format(
                        "%d ## %s ## PersonNotExistsException: Person was not found in the group. ## (surname=%s, name=%s, role=%s, age=%s, email=%s, school=%s, %s)",
                        museumCode, timetable, surname, name, parts[8], parts[4], parts[5], parts[6],
                        parts[8].equalsIgnoreCase("student") ? "studyYear=" + parts[7] : "experience=" + parts[7]
                )));
